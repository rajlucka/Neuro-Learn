...
student's score weight is split equally across all listed concepts. Raw
accumulated weights are then normalised against the maximum achievable
weight per concept to produce mastery values in [0.0, 1.0].
"""
...
def map_scores_to_concepts(
    ...
    """
    Convert a binary question-score matrix into raw per-concept scores.

    For a question covering N concepts, a correct answer contributes 1/N
    to each concept's accumulated score. An incorrect answer contributes 0.
    The raw scores are not yet normalised -- call calculate_mastery() next.
    ...
    Returns:
        Raw accumulated concept scores, shape (students x concepts).
    """
    concept_weights = _build_concept_weight_map(question_metadata)
...
def calculate_mastery(
    ...
    Formula:  mastery = accumulated_correct_weight / maximum_possible_weight

    The maximum possible weight for a concept is the sum of 1/N contributions
    across every question that includes that concept, representing a perfect score.
    """
...
def _build_concept_weight_map(question_metadata: pd.DataFrame) -> dict:
    """Return {question_id: {concept: weight}} for every question."""
    mapping = {}
    for q_id, row in question_metadata.iterrows():
        ...
        mapping[q_id] = {topic: weight for topic in topics}
    return mapping
